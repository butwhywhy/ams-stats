...
#' Analyse AMS data using Linear Perturbation Analysis method proposed by Hext, improved version
#'
...
ams.hext.woN <- function(measures, setup, alpha=0.95){
    ...
    tausError <- .__errors_taus_without(param$eig_param,param$varianza,param$V,N, alpha)
...
#
   ...
   #data <- tausError$taus
   #
   stats <-.__statHext_without(tausError$taus_means, param$eig_param, param$V, param$varianza, param$xb, nrow(measures), alpha)
...
# .__hext_param_without takes a frame with the n measurements and returns a list with a data.frame with the eigenvalues and the eigenvectors, the variance and the trace of susceptibility tensor

.__hext_param_without <- function(measures,setup) {
...
# .__errors_taus_without, takes the eigen-parameters and the variance and returns a list with the eigenvalues and their errors

.__errors_taus_without <- function(eig_param,vari,positions_V,N, alpha){
...
# .__statHext_without takes the eigenvalues, the variance and the trace determines if the anisotropy of magnetic susceptibility (prolate, oblate)

    .__statHext_without <- function(taus, eig_params, V, vari, xb, N, alpha) {
    tau1 = taus[[1]]
    tau2 = taus[[2]]
    tau3 = taus[[3]]
